    ...
    <main class=" flex items-center justify-center h-[calc(100vh-64px)]"> 
        ...
        <h1 class="text-5xl font-bold text-white mb-10">Sooji</h1>
            ...
            <form action="{{ route('logar') }}" method="POST" class>
                ...
                <button type="submit" class=" w-50 text-white m-5 flex items-center align-middle justify-center bg-secondary-color h-10 mt-5 rounded-3xl hover:cursor-pointer">Log in</button>
            
            </form>
            <a href="{{ route('cadastro.index') }}" class="text-white hover:underline">Não possui Cadastro? Clique Aqui</a>
        </div>
